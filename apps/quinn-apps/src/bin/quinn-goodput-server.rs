...
fn process_get(root: &Path, x: &[u8]) -> Result<Vec<u8>> {
    // Support microbench-style "GetN <N>\r\n" requests to return a payload
    // consisting of N bytes of data
    if x.len() >= 5 && &x[..5] == b"GetN " {
        ...
        let request_bytes: usize = s.trim().parse().context("invalid GetN number")?;
        return Ok(vec![0_u8; request_bytes]);
    }
